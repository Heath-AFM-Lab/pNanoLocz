...
class VispyCanvas(scene.SceneCanvas):
    def __init__(self, parent=None):
        # Calculate the size based on the image dimensions
        image_size = (100, 100)  # Since your image is 100x100
        size = (image_size[0], image_size[1])
        
        super().__init__(keys='interactive', parent=parent, size=size, bgcolor='white')
        self.unfreeze()
        ...
        # Set the camera to exactly match the image size
        self.image_view.camera = scene.PanZoomCamera(rect=(0, 0, 1, 1), aspect=1)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Set a fixed size for the window based on the components
        # self.setFixedSize(self.canvas.size[0] + self.colorbar.width() + 20, 
        #                   max(self.canvas.size[1], self.colorbar.height()) + 40)
